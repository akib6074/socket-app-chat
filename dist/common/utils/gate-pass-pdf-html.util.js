...
exports.GRNPDF = exports.mushakPDF = exports.invoicePDF = exports.gatePassHtml = void 0;
const moment = require("moment");
const gatePassHtml = (gatePassData) => {
    let total = 0;
    let count = 0;
    return `<!doctype html>
  <html lang="en">
    ...
    <body>
      ...
      <div class="container text-left mt-4" style="display: none;">
        <div class="row">
          ...
          <div class="col">${gatePassData?.challan?.requisitionId?.toSalesPoint
        ?.salesPointExecutive?.userId}</div>
            ...
            <table class="table table-borderless">
              <tbody>
                ...
                <tr>
                  ...
                  <td>${gatePassData?.challan?.requisitionId?.toSalesPoint
        ?.salesPointExecutive?.userId} </td>
                </tr>
  
                <tr>
                  <td><b>Customer Name :</b></td>
                  <td>${gatePassData?.challan?.requisitionId?.toSalesPoint
        ?.salesPointExecutive?.fullName} </td>
                </tr>
  
                <tr>
                  <td><b>Customer Phone No :</b></td>
                  <td>${gatePassData?.challan?.requisitionId?.toSalesPoint
        ?.salesPointExecutive?.contactNo} </td>
                </tr>
          ...
          <div class="col">
            <table class="table table-borderless">
              <tbody>
                <tr>
                  <td><b>Date :</b></td>
                  <td>${moment(gatePassData?.gatePassDate).format("DD-MM-YYYY")}</td>
                </tr>
      ...
      <table class="table table-striped col table-bordered customTable mt-5 mb-5">
        ...
        <tbody>
        ${gatePassData?.challan?.sku.map((item) => item.skuChallan.map((element, index) => {
        total += 1;
        count += 1;
        return ('<tr style="border-bottom : 2px solid black"><td class="table-data" style="border-right: 2px solid black;border-left: 2px solid black;">' +
            count +
...
exports.gatePassHtml = gatePassHtml;
const invoicePDF = (gatePassData) => {
    let totalAmount = 0;
    return `
  <!DOCTYPE html>
  <html lang="en">
      ...
      <main style="padding: 20px; width: 1200px; margin: auto">
        ...
        <div style="width: 300px; margin: auto;  font-size:20px  ">
          <div style="display: flex; justify-content: space-between">
            <span> <b> Customer Number :</b> </span> <span> ${gatePassData?.challan?.requisitionId?.toSalesPoint
        ?.salesPointExecutive?.userId}</span>
          </div>
          <div style="display: flex; justify-content: space-between">
            <span> <b>Invoice Date :</b></span> <span> ${moment(gatePassData.gatePassDate).format("DD-MM-YYYY")}</span>
          </div>
          <div style="display: flex; justify-content: space-between">
            <span> <b> Invoice Number :</b> </span> <span>${gatePassData.invoiceCode} </span>
          </div>
        </div>
...
<html lang="en">
  ...
  <body>
    <main style="padding: 20px; width: 900px; margin: auto">
      ...
      >
        <div style="flex: 2">
          <p><b> ক্রেতার নাম &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;: </b> ${gatePassData?.challan?.requisitionId?.toSalesPoint
        .salesPointExecutive?.fullName}</p>
...
exports.mushakPDF = mushakPDF;
const GRNPDF = (gatePassData) => {
    return `
  <!DOCTYPE html>
<html lang="en">
  <head>
...
exports.GRNPDF = GRNPDF;
//# sourceMappingURL=gate-pass-pdf-html.util.js.map