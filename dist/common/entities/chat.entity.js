"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ChatEntity = void 0;
const typeorm_1 = require("typeorm");
const custom_base_entity_1 = require("./custom-base.entity");
const user_entity_1 = require("./user.entity");
let ChatEntity = class ChatEntity extends custom_base_entity_1.CustomBaseEntity {
};
exports.ChatEntity = ChatEntity;
__decorate([
    (0, typeorm_1.Column)({ name: 'message', type: 'text', nullable: false }),
    __metadata("design:type", String)
], ChatEntity.prototype, "message", void 0);
__decorate([
    (0, typeorm_1.Column)({ name: 'status', type: 'varchar', nullable: false, default: 'new' }),
    __metadata("design:type", String)
], ChatEntity.prototype, "status", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(type => user_entity_1.UsersEntity, (userEntity) => userEntity.senderMessages),
    (0, typeorm_1.JoinColumn)({ name: 'sender_id' }),
    __metadata("design:type", user_entity_1.UsersEntity)
], ChatEntity.prototype, "sender", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(type => user_entity_1.UsersEntity, (userEntity) => userEntity.receiverMessages),
    (0, typeorm_1.JoinColumn)({ name: 'receiver_id' }),
    __metadata("design:type", user_entity_1.UsersEntity)
], ChatEntity.prototype, "receiver", void 0);
exports.ChatEntity = ChatEntity = __decorate([
    (0, typeorm_1.Entity)({ name: 'co_chats', schema: 'public' }),
    (0, typeorm_1.Index)(['id', 'receiver', 'sender'])
], ChatEntity);
//# sourceMappingURL=chat.entity.js.map