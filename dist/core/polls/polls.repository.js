...
let PollsRepository = PollsRepository_1 = class PollsRepository {
    ...
    async getPoll(pollID) {
        ...
        const key = `polls:${pollID}`;
        try {
            const currentPoll = await this.redisClient.sendCommand(new ioredis_1.Command('JSON.GET', [key]));
    ...
    async addParticipant({ pollID, userID, name, }) {
        ...
        try {
            await this.redisClient.sendCommand(new ioredis_1.Command('JSON.SET', [key,
                participantPath,
                JSON.stringify(name)]));
    ...
    async removeParticipant(pollID, userID) {
        ...
        const participantPath = `.participants.${userID}`;
        try {
            await this.redisClient.sendCommand(new ioredis_1.Command('JSON.DEL', [key, participantPath]));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async removeNomination(pollID, nominationID) {
        ...
        const nominationPath = `.nominations.${nominationID}`;
        try {
            await this.redisClient.sendCommand(new ioredis_1.Command('JSON.DEL', [key, nominationPath]));
            return this.getPoll(pollID);
        }
        catch (e) {
    ...
    async deletePoll(pollID) {
        ...
        try {
            await this.redisClient.sendCommand(new ioredis_1.Command('JSON.DEL', [key]));
        }
        catch (e) {
