...
import { Poll, Results } from './polls-types';
...
export class PollsRepository {
  ...
  async removeParticipant(pollID: string, userID: string): Promise<Poll> {
    ...
    try {
      await this.redisClient.sendCommand(new Command('JSON.DEL', [key, participantPath]));
  ...
  async removeNomination(pollID: string, nominationID: string): Promise<Poll> {
    ...
    try {
      await this.redisClient.sendCommand(new Command('JSON.DEL', [key, nominationPath]));
